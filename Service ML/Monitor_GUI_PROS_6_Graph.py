#!/usr/bin/env python3
"""
Monitor GUI + Jitter + PacketLoss + Memory + CPU + Throughput + Latency
- 6 grafik terpisah
- Packet Loss dikonversi ke persen
- Logs CSV otomatis
- GUI + headless + Telegram snapshot
"""
...
def get_cpu_usage(ip):
    out = run_ssh_command(ip, "top -bn1 | grep 'Cpu(s)' || echo ''", timeout=6)
    if not out: return 0.0
    m = re.search(r"(\d+\.\d+)\s*id", out)
    return round(100.0 - float(m.group(1)), 2) if m else 0.0

def get_mem_usage(ip):
    out = run_ssh_command(ip, "free | grep Mem | awk '{print ($3/$2)*100}'")
    try:
...
def get_iperf_throughput(ip, duration=5, port=None):
    ...
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=duration+20)
...
def get_udp_jitter_loss(ip, duration=5, port=5202):
    ...
    try:
        ...
        loss = end.get("lost_percent", 0.0) * 100  # -> dalam persen nyata
        return round(jitter,3), round(loss,3)
    except Exception as e:
        print(f"[iperf3 UDP error] {ip}: {e}")
        return 0.0, 0.0
...
def get_ping_stats(ip, count=5):
    ...
    try:
        p = subprocess.run(cmd, capture_output=True, text=True, timeout=10+count)
        return _parse_ping_output(p.stdout)
    except Exception as e:
...
def collect_once():
    ...
    ml_tput = get_iperf_throughput(ML_IP)
    f2b_tput = get_iperf_throughput(F2B_IP)
    ml_lat, _, _ = get_ping_stats(ML_IP)
    f2b_lat, _, _ = get_ping_stats(F2B_IP)
    ml_jit, ml_loss = get_udp_jitter_loss(ML_IP)
    f2b_jit, f2b_loss = get_udp_jitter_loss(F2B_IP)
    ml_cpu = get_cpu_usage(ML_IP)
    f2b_cpu = get_cpu_usage(F2B_IP)
    ml_mem = get_mem_usage(ML_IP)
    f2b_mem = get_mem_usage(F2B_IP)
...
def log_to_csv(*values):
    header = ["time","ml_throughput","f2b_throughput","ml_latency","f2b_latency","ml_jitter","f2b_jitter","ml_loss","f2b_loss","ml_cpu","f2b_cpu","ml_mem","f2b_mem"]
